...
test_result = './test_result_grd0.txt'
...
axes.plot(X[::15], testAcc[:2000][::15], 'r-', label='test', linewidth=1.2, )
axes.plot(X[::15], trainAcc[:2000][::15], 'g-', label='train', linewidth=1.2, )
...
plt.savefig(result_image+'/Accuracy_100_grd.svg')
...
axes1.plot(X[::12], testLoss[:2000][::12], 'r-', label='test', linewidth=1.5,)
axes1.plot(X[::12], trainLoss[:2000][::12], 'g-', label='train', linewidth=1.5,)
...
axes1.legend(loc = 'best', shadow=True, fontsize='medium', frameon=True,
                   edgecolor='k')
result_image = './resultPlot'
plt.savefig(result_image+'/Loss_100_grd.svg')
plt.show()
plt.close()
