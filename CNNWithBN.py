...
with g1.as_default():
    ...
    with tf.name_scope('fc1') as scope:
        ...
        #                           initializer=tf.contrib.layers.xavier_initializer(seed=1))
        weight4 = tf.get_variable("weight4", shape=[dim, 128],
                                  initializer=tf.glorot_uniform_initializer())
        bias4 = tf.Variable(tf.constant(0.001, shape=[128]))

        fc1 = (tf.matmul(flatten, weight4) + bias4)
        if phase_train == True:
            fc1 = tf.nn.dropout(fc1, 0.5)
        fc1_bn = batch_normal(fc1, 128, phase_train)
    ...
    with tf.name_scope('fc2') as scope:
        ...
        #                           initializer=tf.contrib.layers.xavier_initializer(seed=1))
        weight5 = tf.get_variable("weight5", shape=[128, 64],
                                  initializer=tf.glorot_uniform_initializer())
        bias5 = tf.Variable(tf.constant(0.0, shape=[64]), trainable=True)
        fc2 = (tf.matmul(fc1_out, weight5) + bias5)
        if phase_train == True:
            fc2 = tf.nn.dropout(fc2, 0.8)
        fc2_bn = batch_normal(fc2, 64, phase_train)
    ...
    with tf.name_scope('output') as scope:
        ...
        #                           initializer=tf.contrib.layers.xavier_initializer(seed=1))
        weight_ = tf.get_variable("weight_", shape=[64, 3],
                                  initializer=tf.glorot_uniform_initializer())
...
print('\n the best test accuracy(auc): %.5f, best loss: %.4f' %(acc_best, test_losses[int(epochElec)]))
...
epoch_seq = np.arange(1, Max_steps+1)
...
ax1.plot(epoch_seq, losses, 'k-', label = 'Training Set')
ax1.plot(epoch_seq, test_losses, 'r-', label = 'Test Set')
ax1.set_title('Loss')
...
ax2.plot(epoch_seq, valid_accs, 'r-', label = 'Valid accuracy')
ax2.plot(epoch_seq, train_accs, 'g-', label = 'Train accuracy')
##set ticks
...
loss_dir = '/home/wzh/PycharmProjects/data/multilabel/multilabel_Xavier/Cross_valid/BNModel/result/loss.txt'
acc_dir = '/home/wzh/PycharmProjects/data/multilabel/multilabel_Xavier/Cross_valid/BNModel/result/accuracy.txt'
